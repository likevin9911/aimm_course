    ...
    contactTime(ros::Time::now())
{
  ...
  this->taskSub = this->node->subscribe("/aimm/task/info", 1,
      &GUITaskWidget::OnTaskInfo, this);
  // Subscribe to wind speed topic (ROS)
  this->windSpeedSub = this->node->subscribe("/aimm/debug/wind/speed", 1,
      &GUITaskWidget::OnWindSpeed, this);
  // Subscribe to wind direction topic (ROS)
  this->windDirectionSub = this->node->subscribe("/aimm/debug/wind/direction", 1,
  ...
  this->contactSub = this->node->subscribe("/aimm/debug/contact", 1,
...
/////////////////////////////////////////////////
void GUITaskWidget::OnContact(const aimm_gazebo::Contact::ConstPtr &_msg)
{
  ...
  // Write what the lpv is in contact with
...
{
  // find lpv base_link in all the links reported
  unsigned int c = 0;
  ...
  double roll, pitch;
  // update lpvHeading
  m.getRPY(roll, pitch, this->lpvHeading);
...
void GUITaskWidget::OnWindDirection(const std_msgs::Float64::ConstPtr &_msg)
{
  ...
  this->windPainter.setPen(this->pen);
  x = (-40*cos(this->lpvHeading + pi)) + 75;
  y = (40*sin(this->lpvHeading + pi)) + 75;
  this->windPainter.drawLine(QLine(75, 75, x, y));
...
/////////////////////////////////////////////////
void GUITaskWidget::OnTaskInfo(const aimm_gazebo::Task::ConstPtr &_msg)
{
