...
#include <ros/ros.h>
#include <aimm_gazebo/Task.h>
#include <aimm_gazebo/Contact.h>
...
namespace gazebo
{
  class GUITaskWidget : public GUIPlugin
  {
    ...
    /// \brief A signal used to set the wind and lpv direction
    ...
    protected: void OnTaskInfo(const aimm_gazebo::Task::ConstPtr &_msg);
    ...
    /// \param[in] _msg wind direction info message that is received.
    protected: void OnContact(const aimm_gazebo::Contact::ConstPtr &_msg);
    ...
    /// \brief Pixmap for the wind and lpv direction compass
    ...
    /// \brief last reported lpvHeading(yaw)
    private: double lpvHeading = 0;
  };
}
