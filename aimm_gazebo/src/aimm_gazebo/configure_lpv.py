...
from aimm_gazebo.compliance import ComponentCompliance
from aimm_gazebo.compliance import ThrusterCompliance

from aimm_gazebo.utils import create_xacro_file
from aimm_gazebo.utils import add_gazebo_thruster_config


def main():

    rospy.init_node("lpv_generator", anonymous=True)
    ...
    create_urdf_command = ("rosrun xacro xacro -o " + lpv_target +
                           " '" + lpv_gazebo + "'")
...
def create_thruster_xacro():
    ...
    thruster_boiler_plate_top = ('<?xml version="1.0"?>\n'
                                 ...
                                 '"$(find lpv_description)/urdf/thrusters/'
                                 'engine.xacro" />\n')
    ...
    gz_boiler_plate_top = ('  <gazebo>\n'
                           '    <plugin name="lpv_gazebo_thrust" '
                           'filename="libusv_gazebo_thrust_plugin.so">\n'
                           '      <cmdTimeout>1.0</cmdTimeout>\n'
                           '      <robotNamespace>${namespace}</robotNamespace>\n'
                           '      <xacro:include filename="$(find lpv_gazebo)'
                           '/urdf/thruster_layouts/'
                           'lpv_gazebo_thruster_config.xacro" />\n')
