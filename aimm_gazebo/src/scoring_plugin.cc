...
#include "aimm_gazebo/scoring_plugin.hh"
    ...
    sdf::ElementPtr _sdf)
{
  ...
  this->taskPub = this->rosNode->advertise<aimm_gazebo::Task>
    (this->taskInfoTopic, 100);
  this->contactPub = this->rosNode->advertise<aimm_gazebo::Contact>
...
void ScoringPlugin::OnCollisionMsg(ConstContactsPtr &_contacts)
  {
  // loop through collisions, if any include the lpv, increment collision
  // counter
  for (unsigned int i = 0; i < _contacts->contact_size(); ++i)
  {
    std::string lpvCollisionStr1 = _contacts->contact(i).collision1();
    std::string lpvCollisionStr2 = _contacts->contact(i).collision2();
    std::string lpvCollisionSubStr1 =
        lpvCollisionStr1.substr(0, lpvCollisionStr1.find("lump"));
    std::string lpvCollisionSubStr2 =
        lpvCollisionStr2.substr(0, lpvCollisionStr2.find("lump"));

    bool isWamvHit =
      lpvCollisionSubStr1 == "lpv::base_link::base_link_fixed_joint_" ||
      lpvCollisionSubStr1 ==
        "lpv::lpv/base_link::lpv/base_link_fixed_joint_"             ||
      lpvCollisionSubStr2 == "lpv::base_link::base_link_fixed_joint_" ||
      lpvCollisionSubStr2 ==
        "lpv::lpv/base_link::lpv/base_link_fixed_joint_";
