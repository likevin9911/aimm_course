...
#include "aimm_gazebo/scan_dock_scoring_plugin.hh"
    ...
    aimm_gazebo::ColorSequence::Response> &_event)
{
  ...
  const aimm_gazebo::ColorSequence::Request &req = _event.getRequest();
  aimm_gazebo::ColorSequence::Response &res = _event.getResponse();
...
bool ScanDockScoringPlugin::ParseSDF(sdf::ElementPtr _sdf)
{
  ...
  std::string colorSequenceService = "/aimm/scan_dock/color_sequence";
  ...
  {
    this->colorTopic = "/aimm/light_buoy/new_pattern";
  }
  else
  {
