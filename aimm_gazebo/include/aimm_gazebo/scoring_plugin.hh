...
/// topic name is /aimm/task/info .
///
/// <contact_debug_topic>: Optional parameter (string type)
/// containing the ROS topic name to
/// publish every instant a collision with the lpv is happening.
/// Default is /aimm/debug/contact.
///
...
class ScoringPlugin : public gazebo::WorldPlugin
{
  ...
  private: std::string taskInfoTopic = "/aimm/task/info";

  /// \brief Bool flag for debug.
  private: bool debug = true;

  /// \brief Topic where debug collision is published.
  private: std::string contactDebugTopic = "/aimm/debug/contact";
  ...
  protected: aimm_gazebo::Task taskMsg;

  /// \brief ROS Contact Msg.
  private: aimm_gazebo::Contact contactMsg;
