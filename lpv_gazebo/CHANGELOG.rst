^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package lpv_gazebo
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
* Style changes.
* Merge
* Added commented code to lpv_xacro
* removed from lpv
...
* added force visualization to lpv
...
1.1.0 (2019-07-01)
------------------
* lpv_gps.xacro edited online with Bitbucket
...
* Update lpv_gazebo.urdf.xacro file to use thruster yaml file if given
...
* Add mono_camera mesh to urdf file and onto LPV-4
* Merged default into Issue#86-add-3d-lidar-mesh
* Add sensor_post_arm.dae
* Add sensor post to 3d lidar on LPV-4, including height parameter
...
* Merge from default.
* Merged in aimm (pull request #68)
  Rename vmrc to aimm
...
* Merged in urdf_easy (pull request #62)
  ...
  Approved-by: Brian Bingham <briansbingham@gmail.com>
* Simplify urdf files.
* Locking the LPV-4 conditionally.
...
* Add the pinger plugin to the lpv_gazebo package.
  The lpv_gazebo_sensors.urdf file has been modified to add support for the pinger plugin.
* add missing dependencies
* Create perception.launch and lock the LPV-4.
...
* trying to get lpv to be static using a fixed joint
* Merge from default.
...
* Fix issues after lpv_gazebo migration
* Boostrap lpv_gazebo
